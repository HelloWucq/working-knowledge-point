...
#十一.诊断网络问题
##11.1.ping命令（因特网包探索器）
- ping 127.0.0.1:如果测试成功，表明网卡、TCP/IP协议的安装、IP地址、子网掩码的设置正常。如果测试不成功，就表示TCP/IP的安装或设置存在问题
- ping 本机IP地址：如果测试不成功，表示本地配置或安装存在问题，应当对网络设备或通讯介质进行测试、检查并排除
- ping 局域网内其他IP：如果测试成功，表明本地网络中的网卡和载体运行正确。但如果收到0个回送应答，那么表示子网掩码不正确或网卡配置错误或电缆有问题
- ping 网关IP:这个命令如果应答正确，表示局域网中的网关路由器正在运行并能够做出应答
- ping 远程IP:如果收到正确应答，表示成功的使用缺省网关

##11.2.ipconfig:用于显示当前TCP/IP配置的设置值

##11.3.arp命令（地址转换协议）：用于确定对应IP地址的网卡物理地址

##11.4.traceroute：测量路由情况，即用来显示数据包到达目的地主机所经过的路径

##11.5.route:用来显示、人工添加和修改路由表项目的